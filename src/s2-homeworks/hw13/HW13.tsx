...
const HW13 = () => {
  ...
  const responseHandler = (text: string, info: string) => {
    setText(text);
    setInfo(info);
  };

  const send = (x?: boolean | null) => () => {
    const url: string = x === null ? "https://xxxxxx.ccc" : "https://samurai.it-incubator.io/api/3.0/homework/test";
    setIsLoading(true);
    setCode("");
    setImage("");
    setText("");
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setCode("Код 200!");
          setImage(success200);
          responseHandler(res.data.errorText, res.data.info);
        }
      })
      .catch((e) => {
        if (e.response.status === 500) {
          setCode("Ошибка 500!");
          setImage(error500);
          responseHandler(e.response.data.errorText, e.response.data.info);
          ...
          responseHandler(e.message, e.name);
        }
      })
      .finally(() => {
        setIsLoading(false);
